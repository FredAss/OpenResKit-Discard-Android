    ...
    public void onDraw(Canvas ca) {
        ...
        if (fullImage == null || shouldUpdate) {
            ...
            if (showBarText) {
                ...
                usableHeight = getHeight() - bottomPadding - topPadding - Math.abs(r3.top - r3.bottom) - 26;
            } else {
                usableHeight = getHeight() - bottomPadding - topPadding;
            }
            ...
            for (Bar p : points) {
                ...
                if (showBarText) {
                    ...
                    if (p.isShowStringValue()) 
                    {
                    	 this.p.getTextBounds(unit + p.getStringValue(), 0, 1, r2);
                    	if (popup != null) 
                    	{
                    		//popup.setBounds((int) (((r.left + r.right) / 2) - (this.p.measureText(p.getStringValue()) / 2)) - 14, r.top + (r2.top - r2.bottom) - 26, (int) (((r.left + r.right) / 2) + (this.p.measureText(p.getStringValue()) / 2)) + 14, r.top);
                         	//popup.draw(canvas);
                         	canvas.drawText(p.getStringValue(), (int) (((r.left + r.right) / 2) - (this.p.measureText(p.getStringValue()) / 2)), r.top - 20, this.p);
                    	}
                    }
                    else 
                    {
